...
pub fn render(pal: &Palette, frame: &mut Frame, area: Rect) {
    ...
    let lines = vec![
        ...
        Line::from("  y         sync favourites"),
        Line::from("  [ ]       volume"),
        Line::from(""),
        Line::from("SYNC (across devices)"),
        Line::from("  link once from a terminal:"),
        Line::from("    world-radio sync login   → shows key + qr"),
        Line::from("  scan the qr in the phone app to pair"),
        Line::from("  y here pushes + pulls favourites (union merge)"),
        Line::from("  world-radio sync status / logout   manage the key"),
        Line::from("  no key = fully local"),
        Line::from(""),
